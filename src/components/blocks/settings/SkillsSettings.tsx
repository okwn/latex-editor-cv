...
import { SkillsSettings, SkillsStyle, SkillsColumns, SkillsSpacing } from '@/types/blockLayout';
...
export function SkillsSettingsForm({ block }: Props) {
  const { resumeData, updateResumeData } = useEditorStore();
  const settings = (block.settings as unknown as SkillsSettings) || { style: 'grouped-lines', columns: 2, spacing: 'normal' };

  const handleChange = useCallback(
    (key: keyof SkillsSettings, value: unknown) => {
  ...
  return (
    <div className="space-y-4">
      <div>
        <label className="block text-xs font-medium text-zinc-300 mb-1.5">Style</label>
        <div className="flex gap-1">
          {(['grouped-lines', 'chips', 'compact'] as SkillsStyle[]).map((s) => (
            <button
              key={s}
              onClick={() => handleChange('style', s)}
              className={`flex-1 px-2 py-1.5 rounded text-xs capitalize transition-colors ${
                settings.style === s ? 'bg-blue-600 text-white' : 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700'
              }`}
            >
              {s.replace('-', ' ')}
            </button>
      ...
      <div>
        ...
        <div className="flex gap-1">
          {(['compact', 'normal', 'relaxed'] as SkillsSpacing[]).map((s) => (
            <button
              key={s}
