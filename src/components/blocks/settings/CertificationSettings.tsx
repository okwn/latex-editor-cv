...
import { CertificationSettings, CertificationColumns, CertificationSpacing } from '@/types/blockLayout';
...
export function CertificationSettingsForm({ block }: Props) {
  const { resumeData, updateResumeData } = useEditorStore();
  const settings = (block.settings as unknown as CertificationSettings) || { columns: 3, spacing: 'normal' };

  const handleChange = useCallback(
    (key: keyof CertificationSettings, value: unknown) => {
    ...
    <div className="space-y-4">
      <div>
        ...
        <div className="flex gap-1">
          {([1, 2, 3] as CertificationColumns[]).map((n) => (
            <button
              key={n}
      ...
      <div>
        ...
        <div className="flex gap-1">
          {(['compact', 'normal', 'relaxed'] as CertificationSpacing[]).map((s) => (
            <button
              key={s}
