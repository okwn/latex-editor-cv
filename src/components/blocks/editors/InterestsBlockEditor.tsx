...
export function InterestsBlockEditor({ blockId }: { blockId: string }) {
  ...
  if (!block || block.type !== 'interests') return null;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <label className="block text-xs font-medium text-zinc-300 mb-1.5">Interests</label>
        {items.length === 0 && (
          <p className="text-xs text-zinc-500 italic mb-2">This block is added but empty. Add your first interest.</p>
        )}
        {items.map((item, idx) => (
          <div key={idx} className="flex gap-2 items-center">
            <input type="text" value={item} onChange={(e) => handleItemChange(idx, e.target.value)}
              placeholder="Open Source, Hiking, Photography..." className="flex-1 bg-zinc-800 border border-zinc-700 rounded px-2 py-1.5 text-xs text-zinc-200 focus:outline-none focus:border-zinc-600" />
            {items.length > 1 && (
